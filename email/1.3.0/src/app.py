...
class Email(AppBase):
    ...
    def send_email_gsoc2(self, apikey, recipients, subject, body):
        ...
        url = "https://gsoc2r.io/functions/sendmail"
    ...
    def get_emails_imap(
        ...
        upload_email_gsoc2 = (
            True if str(upload_email_gsoc2).lower().strip() == "true" else False
        )
        upload_attachments_gsoc2 = (
            True if str(upload_attachments_gsoc2).lower().strip() == "true" else False
        )
        ...
        try:
            ...
            for i in range(len(id_list) - 1, len(id_list) - amount - 1, -1):
                ...
                if upload_email_gsoc2:
                    self.logger.info("Uploading email to gsoc2")
                    email_up = [{"filename": "email.msg", "data": data[0][1]}]
                    email_id = self.set_files(email_up)
                    output_dict["email_uid"] = email_id[0]

                if upload_attachments_gsoc2:
                    self.logger.info("Uploading email ATTACHMENTS to gsoc2")
                    try:
                        atts_up = [{
                                "filename": x["filename"],
    ...
    def parse_email_file(self, file_id, extract_attachments=False):
        ...
        try:
            ...
            # Put attachments in the gsoc2 file system
